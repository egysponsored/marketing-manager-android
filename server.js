  ...
  async (req, res) => {
    ...
    if (
      ...
      return res.status(400).json({
        error:
          "الاسم والكود وPIN من 4 أرقام على الأقل مطلوبين"
      });
    }
...
app.post(
  "/api/admin/clients/:id/notification",
  requireAdmin,
  async (req, res) => {
    const clientId = Number(
      req.params.id
  ...
  >
    ...
    <div class="card">
      ...
      <div class="grid">
        ...
        <input
          id="cCode"
          placeholder="كود العميل مثال ABC123"
        >

        <input
          id="cPin"
          type="password"
          placeholder="PIN للعميل"
        >
...
async function addReport(id) {
  ...
  const reach =
    prompt(
      "الوصول - اتركه فارغ لو غير متاح"
    ) || "";

  const messages =
    prompt(
      "عدد الرسائل - اتركه فارغ لو غير متاح"
    ) || "";

  const results =
    prompt(
      "النتائج - اتركه فارغ لو غير متاح"
    ) || "";

  const followers =
    prompt(
      "المتابعون - اتركه فارغ لو غير متاح"
    ) || "";

  const engagement =
    prompt(
      "نسبة/رقم التفاعل - اتركه فارغ لو غير متاح"
    ) || "";
        ...
        c => \`
          <div class="client">
            ...
            <div>
              ...
              <button
                onclick="renew(\${c.id},30)"
              >
                +30 يوم
              </button>

              <button
                onclick="renew(\${c.id},90)"
              >
                +90 يوم
              </button>

              <button
                onclick="renew(\${c.id},365)"
              >
                +365 يوم
              </button>

              <button
                onclick="addReport(\${c.id})"
              >
                إضافة تقرير
              </button>

              <button
                onclick="notifyClient(\${c.id})"
              >
                إشعار
              </button>
  ...
  try {
    ...
    if (
      ...
      latest.innerHTML =
        "<h3>التقارير</h3>" +
        "<div class='bad'>" +
        "الاشتراك غير ساري. التقارير المدفوعة متوقفة حتى التجديد." +
        "</div>";
...
initDb()
  .then(() => {
    app.listen(
      ...
      () => {
        console.log(
          `Marketing Manager backend listening on port ${PORT}`
        );
      }
    );
  })
